...
import org.fibs.geotag.track.TrackMatcher.TrackPointComparator;
...
public final class TrackStore {
	...
	private final Map<Trkseg, BoundsType> segmentBounds = new HashMap<>();
	 ...
	 * @param newGpx
	 */
	public void addGPX(final Gpx newGpx) {
		if (gpx == null) {
			gpx = newGpx;
		} else {
		...
		final List<Trkseg> newSegmentList = new ArrayList<>();
		for (final Trk track : gpx.getTrk()) {
			for (final Trkseg segment : track.getTrkseg()) {
				final List<Trkpt> trackpointList = new ArrayList<>();
				for (final Trkpt trackpoint : segment.getTrkpt()) {
		...
		for (final Trkseg segment : newSegmentList) {
			if (lastUniqueSegment == null || comparator.compare(lastUniqueSegment, segment) != 0) {
				Collections.sort(segment.getTrkpt(), new TrackPointComparator());
	 ...
	 * @param segments
	 */
	private void createTrackSegmentBounds(final List<Trkseg> segments) {
		// unfortunately GPX only defines a bounds for the entire file, not per
		// track
		segmentBounds.clear();
		final ObjectFactory objectFactory = new ObjectFactory();
		for (final Trkseg segment : segments) {
			if (segment.getTrkpt().size() > 0) {
				// we calculate bounds for each track segment
				final BoundsType bounds = objectFactory.createBoundsType();
				for (final Trkpt trackpoint : segment.getTrkpt()) {
					final BigDecimal latitude = trackpoint.getLat();
					final BigDecimal longitude = trackpoint.getLon();
					...
					if (bounds.getMaxlon() == null || longitude.compareTo(bounds.getMaxlon()) > 0) {
	 ...
	 */
	BoundsType getSegmentBounds(final Trkseg segment) {
	 ...
	 */
	public List<Trkseg> getIntersectingTrackSegments(final BoundsType mapBounds) {
		final List<Trkseg> result = new ArrayList<>();
		if (gpx != null) {
			for (final Trk track : gpx.getTrk()) {
				for (final Trkseg segment : track.getTrkseg()) {
					final BoundsType bounds = segmentBounds.get(segment);
					if (bounds != null) {
	...
	/**
	 * A class for comparing timestamps of tracks segments. Only the timestamp
	 * of the first trackpoint is compared.
	 */
	static class SegmentComparator implements Comparator<Trkseg>, Serializable {
		 ...
		 */
		@Override
		public int compare(final Trkseg segment1, final Trkseg segment2) {

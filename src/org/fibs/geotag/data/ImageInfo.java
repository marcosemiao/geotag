...
public class ImageInfo implements Comparable<ImageInfo> {
	...
	/**
	 * Defines how date/time is formatted in EXIF terms. Do not use static date
	 * formats. They are not thread safe and should not be shared.
	 */
	...
	private final Calendar exactTimeGMT = Calendar.getInstance(TimeZone.getTimeZone("GMT")); //$NON-NLS-1$
	 ...
	 */
	public ImageInfo(final File file) {
	 ...
	 * . The offset is calculated such that GMT time plus offset equals local
	 * time
	 *
	 * @param gmtDateString
	 ...
	 */
	public int calculateOffset(final String gmtDateString, final String localDateString) {
		int offset = 0;
		if (gmtDateString != null && localDateString != null) {
			try {
				final Date gmtDate = dateFormat.parse(gmtDateString);
				final Date localDate = dateFormat.parse(localDateString);
	 ...
	 */
	public String subtractOffset(final String localDateString, final int offset) {
		try {
			final Date date = dateFormat.parse(localDateString);
	 ...
	 */
	@Override
	public int compareTo(final ImageInfo imageInfo) {
	 ...
	 */
	public static ImageInfo getImageInfo(final int sequenceNumer) {
	 ...
	 */
	public static List<ImageInfo> getImagesInfos(final Filter filter) {
		final List<ImageInfo> result = new ArrayList<>();
		for (final ImageInfo imageInfo : instances) {
	 ...
	 */
	public static ImageInfo getImageInfo(final String filePath) {
	...
	public boolean hasGps() {
		if (gpsLatitude != null && gpsLatitude.length() > 0) {
			return true;
		}
		if (gpsLongitude != null && gpsLongitude.length() > 0) {
			return true;
		}
		if (gpsAltitude != null && gpsAltitude.length() > 0) {
			return true;
		}
		return false;
	}
	 ...
	 */
	public void setThumbNailStatus(final THUMBNAIL_STATUS thumbNailStatus) {
	 ...
	 */
	public void setThumbnail(final ImageIcon thumbnail) {
	 ...
	 */
	public static String getOffsetString(final int offset) {
		...
		final int minutes = (absOffset / (int) Constants.MINUTES_PER_HOUR);
		final int seconds = (absOffset - minutes * (int) Constants.SECONDS_PER_MINUTE);
		hours *= negative ? -1 : 1;
		return String.format("%+d:%02d:%02d", Integer.valueOf(hours), Integer.valueOf(minutes), //$NON-NLS-1$
	 ...
	 */
	void setCameraDate(final String cameraDate) {
	 ...
	 */
	void setGpsLatitude(final String latitude, final DATA_SOURCE source) {
	 ...
	 */
	void setGpsLongitude(final String longitude, final DATA_SOURCE source) {
	 ...
	 *            Where the position data comes from
	 */
	void setGpsAltitudeInMetres(final String altitude, final DATA_SOURCE source) {
	 ...
	 */
	void setGpsImgDirection(final String direction) {
	 ...
	 */
	void setGpsDateTime(final String dateTime) {
		if (dateTime != null) {
			try {
				final Date parsedDate = dateFormat.parse(dateTime);
	...
	public void setGpsDateTime() {
		if (gpsDateTime == null && cameraDate != null) {
			...
			final DateFormat format = new SimpleDateFormat(ImageInfo.getDateFormatPattern());
			try {
				// parse the date as local time zone
				final Date date = format.parse(cameraDate);
				// and format it back to GMT
				format.setTimeZone(TimeZone.getTimeZone("GMT")); //$NON-NLS-1$
				final String gmtTime = format.format(date);
				// update
	 ...
	 * @param source
	 */
	void setLocationName(final String locationName, final DATA_SOURCE source) {
	 ...
	 * @param source
	 */
	void setCityName(final String cityName, final DATA_SOURCE source) {
	 ...
	 * @param source
	 */
	void setProvinceName(final String provinceName, final DATA_SOURCE source) {
	 ...
	 * @param source
	 */
	void setCountryName(final String countryName, final DATA_SOURCE source) {
	 ...
	 * @param source
	 */
	void setUserComment(final String userComment, final DATA_SOURCE source) {
	 ...
	 */
	public void setNearbyLocations(final List<Location> nearbyLocations) {
	 ...
	 * @param source
	 */
	public void setSource(final DATA_SOURCE source) {
		this.source = source;
	}

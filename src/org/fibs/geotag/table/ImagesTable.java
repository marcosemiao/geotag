...
public class ImagesTable extends NavigableTable {
	...
	/**
	 * We keep the last mouse motion event. We need it so we can dispatch it
	 * again to redraw the tool tip. This idea was found at:
	...
	private final Color alternativeBackground;
	 ...
	 *
	 * @param tableModel
	 *            the {@link ImagesTableModel} containing the data to be
	 *            displayed
	 */
	public ImagesTable(final ImagesTableModel tableModel) {
		super(tableModel);
		...
		final SettingsListener settingsListener = setting -> {
			// no need really to check
			final int clicksToEdit = Settings.get(SETTING.CLICKS_TO_EDIT, Settings.DEFAULT_CLICKS_TO_EDIT);
			((DefaultCellEditor) getDefaultEditor(String.class)).setClickCountToStart(clicksToEdit);
		};
		// kick it once
	 ...
	 * http://bugs.sun.com/bugdatabase/view_bug.do?bug_id=6503981 This bug
	 * should be fixed soon and this method might need to check the Java version
	 * to apply the fix selectively.
	 */
	private void fixBug6503981() {
		...
		header.addMouseListener(new MouseAdapter() {
			...
			public void mousePressed(final MouseEvent event) {
				final int column = header.getColumnModel().getColumnIndexAtX(event.getPoint().x);
				if (column >= 0) {
					ImagesTable.this.setColumnSelectionInterval(column, column);
					final Action focusAction = ImagesTable.this.getActionMap().get("focusHeader"); //$NON-NLS-1$
					focusAction.actionPerformed(new ActionEvent(ImagesTable.this, 0, "focusHeader")); //$NON-NLS-1$
				}
			}
		});
	}
	...
	private void setupActions() {
		getInputMap(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT)
				.put(KeyStroke.getKeyStroke(KeyEvent.VK_DOWN, 0, false), "downArrow"); //$NON-NLS-1$
		final AbstractAction downArrow = new AbstractAction() {
			@Override
		...
		getInputMap(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT).put(KeyStroke.getKeyStroke(KeyEvent.VK_UP, 0, false),
				"upArrow"); //$NON-NLS-1$
		final AbstractAction upArrow = new AbstractAction() {
			@Override
			public void actionPerformed(final ActionEvent evt) {
		...
		getInputMap(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT)
				.put(KeyStroke.getKeyStroke(KeyEvent.VK_LEFT, 0, false), "leftArrow"); //$NON-NLS-1$
		final AbstractAction leftArrow = new AbstractAction() {
			@Override
			public void actionPerformed(final ActionEvent evt) {
		...
		getInputMap(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT)
				.put(KeyStroke.getKeyStroke(KeyEvent.VK_RIGHT, 0, false), "rightArrow"); //$NON-NLS-1$
		final AbstractAction rightArrow = new AbstractAction() {
			@Override
			public void actionPerformed(final ActionEvent evt) {
	...
	private Color calculateAlternativeBackgroundColour() {
		final double ratio = ALTERNATIVE_BACKGROUND_RATIO; // 10 % seems just
															// about right
		final int red = (int) Util.applyRatio(getBackground().getRed(), getForeground().getRed(), ratio);
		final int green = (int) Util.applyRatio(getBackground().getGreen(), getForeground().getGreen(), ratio);
		final int blue = (int) Util.applyRatio(getBackground().getBlue(), getForeground().getBlue(), ratio);
	 ...
	 */
	private SETTING getColumnWidthSetting(final ImagesTableColumns.COLUMN column) {
	 ...
	 * @return The {@link SETTING}
	 */
	private SETTING getColumnPositionSetting(final ImagesTableColumns.COLUMN column) {
	...
	/**
	 * This method lays out the columns to either a sensible default layout or
	 * one previously save to the Settings.
	 */
	private void determineColumnLayout() {
		...
		final String[] correctOrder = new String[ImagesTableColumns.COLUMN.values().length];
		for (int index = 0; index < ImagesTableColumns.COLUMN.values().length; index++) {
			final COLUMN column = ImagesTableColumns.COLUMN.values()[index];
			...
			final String columnName = getModel().getColumnName(index);
			final int columnIndex = getColumnModel().getColumnIndex(columnName);
			...
			final int newColumnIndex = Settings.get(setting, columnIndex);
		...
		for (int index = 0; index < correctOrder.length; index++) {
			final String columnName = correctOrder[index];
			final int currentIndex = getColumnModel().getColumnIndex(columnName);
	...
	public void saveColumnSettings() {
		for (int index = 0; index < COLUMN.values().length; index++) {
			...
			final String columnName = getModel().getColumnName(index);
			final int columnIndex = getColumnModel().getColumnIndex(columnName);
			final COLUMN column = COLUMN.values()[index];
			SETTING setting = getColumnWidthSetting(column);
			if (setting != null) {
				final int width = getColumnModel().getColumn(columnIndex).getPreferredWidth();
	 ...
	 */
	private int defaultImageNameWidth(final FontMetrics fontMetrics) {
	 ...
	 */
	private int defaultOffsetWidth(final FontMetrics fontMetrics) {
	 ...
	 */
	private int defaultLongitudeWidth(final FontMetrics fontMetrics) {
	 ...
	 */
	private int defaultLatitudeWidth(final FontMetrics fontMetrics) {
	 ...
	 */
	private int defaultDateWidth(final FontMetrics fontMetrics) {
	 ...
	 */
	private int defaultAltitudeWidth(final FontMetrics fontMetrics) {
	 ...
	 */
	private int defaultDirectionWidth(final FontMetrics fontMetrics) {
	 ...
	 */
	private int defaultLocationNameWidth(final FontMetrics fontMetrics) {
	 ...
	 */
	private int defaultCityNameWidth(final FontMetrics fontMetrics) {
	 ...
	 */
	private int defaultProvinceNameWidth(final FontMetrics fontMetrics) {
	 ...
	 */
	private int defaultCountryNameWidth(final FontMetrics fontMetrics) {
	 ...
	 */
	private int defaultUserCommentWidth(final FontMetrics fontMetrics) {
	...
	public Component prepareRenderer(final TableCellRenderer renderer, final int row, final int column) {
		...
		final ImageInfo imageInfo = ((ImagesTableModel) getModel()).getImageInfo(row);
		...
		// set the renderer font to either the table font or the table font in
		// bold
	...
	public void usePreferredFont() {
		final String fontId = Settings.get(SETTING.FONT, null);
		if (fontId != null) {
			final Font font = FontUtil.fontFromID(Settings.get(SETTING.FONT, null));
			if (font != null) {
				setFont(font);
				textCellEditor.setFont(font);
				int borderInsets = 0;
				final Border border = textCellEditor.getBorder();
				if (border != null) {
					final Insets insets = border.getBorderInsets(textCellEditor);
	...
	public JToolTip createToolTip() {
		final ImageInfo imageInfo = ((ImagesTableModel) getModel()).getImageInfo(mouseOnRow);
		// It's OK if imageIcon is null at this stage, the tooltip constructor
		// doesn't mind
		final ImageIcon imageIcon = imageInfo.getThumbnail();
		final ImageToolTip tooltip = new ImageToolTip(imageIcon, imageInfo.getPath());
		// only try loading thumbnail once
		final boolean showThumbnails = Settings.get(SETTING.TUMBNAILS_IN_TOOLTIPS, true);
		if (imageIcon == null && showThumbnails && imageInfo.getThumbNailStatus() == THUMBNAIL_STATUS.UNKNOWN) {
			final ThumbnailWorker worker = new ThumbnailWorker(imageInfo) {
				@Override
				protected void process(final List<ImageInfo> chunks) {
	...
	public void selectAllWithNoGps() {
		clearSelection();
		for (int row = 0; row < getRowCount(); row++) {
			final ImageInfo imageInfo = ((ImagesTableModel) getModel()).getImageInfo(row);
			if (!imageInfo.hasGps()) {
	...
	/**
	 * We override this, because we want a custom TableHeader displaying
	...
	protected JTableHeader createDefaultTableHeader() {
		final JTableHeader defaultTableHeader = new JTableHeader(columnModel) {
			@Override
			public String getToolTipText(final MouseEvent e) {
				final int columnIndex = columnModel.getColumnIndexAtX(e.getPoint().x);
				final int modelIndex = columnModel.getColumn(columnIndex).getModelIndex();
				final COLUMN column = COLUMN.values()[modelIndex];
	 ...
	 */
	void setLastMouseMovedEvent(final MouseEvent lastMouseMovedEvent) {
	 ...
	 */
	void setMouseOnRow(final int mouseOnRow) {
